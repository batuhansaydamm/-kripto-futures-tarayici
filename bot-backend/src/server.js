...
<meta name="theme-color" content="#080b10"><title>V13.4 Bot</title>
<style>
...
<div class="head"><div class="eyebrow">V13.4 · STREAM SCANNER · BINANCE FUTURES TESTNET</div><h1>Bot Kontrol Paneli</h1><div class="dim">50 USDT · x10 · isolated · toplam 5 işlem · eşzamanlı 1</div></div>
...
<div id="app" hidden>
 <div class="card"><span class="pill" id="mode"></span> <span class="pill" id="enabled"></span> <span class="pill" id="kill"></span> <span class="pill" id="stream"></span></div>
 <div class="grid">
...
function publicState(store, config, bot) {
  return {
    config: {
    ...
    busy: bot.busy,
    marketStream: bot.marketClient.health?.() || null,
    state: store.state,
  };
}
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      if (req.method === "GET" && url.pathname === "/api/status")
        return json(res, 200, publicState(store, config, bot));
